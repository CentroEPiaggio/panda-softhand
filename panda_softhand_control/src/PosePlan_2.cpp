...
PosePlan_2::PosePlan_2(ros::NodeHandle& nh_, std::string group_name_, std::string end_effector_name_){
        
        ROS_INFO("Starting to create PosePlan_2 object");
        ...
        ROS_INFO("Finished creating PosePlan_2 object");
}

PosePlan_2::~PosePlan_2(){
    
    // Nothing to do here yet
}

// This is the callback function of the pose plan service
bool PosePlan_2::call_pose_plan(panda_softhand_control::pose_plan::Request &req, panda_softhand_control::pose_plan::Response &res){

    // Setting up things
    if(!this->initialize(req.goal_pose, req.start_pose, req.is_goal_relative, req.past_trajectory)){
        ROS_ERROR("Could not initialize PosePlan_2 object. Returning...");
        res.answer = false;
        return false;
    }
    ...
    if(!this->performMotionPlan()){
        ROS_ERROR("Could not perform motion planning in PosePlan_2 object. Returning...");
        res.answer = false;
        return false;
    }
...
bool PosePlan_2::initialize(geometry_msgs::Pose goal_pose, geometry_msgs::Pose start_pose, bool is_goal_relative, trajectory_msgs::JointTrajectory past_trajectory){
...
bool PosePlan_2::performMotionPlan(){
