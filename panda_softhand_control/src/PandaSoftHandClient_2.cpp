...
PandaSoftHandClient_2::PandaSoftHandClient_2(){

    // Nothing to do here
}

PandaSoftHandClient_2::PandaSoftHandClient_2(ros::NodeHandle& nh_){

    // Initializing object
    if(!this->initialize(nh_)){
        ROS_ERROR("The PandaSoftHandClient_2 was not initialized successfully. Some servers are missing...");
        ros::shutdown();
    } 
}
...
bool PandaSoftHandClient_2::initialize(ros::NodeHandle& nh_){
    ...
    this->hand_plan_service_2_name = "hand_plan_service_2";
    this->hand_control_service_2_name = "hand_control_service_2";
    this->hand_wait_service_2_name = "hand_wait_service_2";

    this->arm_control_service_2_name = "arm_control_service_2";
    this->arm_wait_service_2_name = "arm_wait_service_2";

    this->joint_service_2_name = "joint_plan_service_2";
    this->pose_service_2_name = "pose_plan_service_2";
    this->slerp_service_2_name = "slerp_plan_service_2";

    // Initializing service clients after waiting
    if(!ros::service::waitForService(this->hand_plan_service_2_name, ros::Duration(1.0))) return false;
    this->hand_plan_client = this->nh.serviceClient<panda_softhand_control::hand_plan>(this->hand_plan_service_2_name);

    if(!ros::service::waitForService(this->hand_control_service_2_name, ros::Duration(1.0))) return false;
    this->hand_control_client = this->nh.serviceClient<panda_softhand_control::hand_control>(this->hand_control_service_2_name);

    if(!ros::service::waitForService(this->hand_wait_service_2_name, ros::Duration(1.0))) return false;
    this->hand_wait_client = this->nh.serviceClient<panda_softhand_control::hand_wait>(this->hand_wait_service_2_name);

    if(!ros::service::waitForService(this->arm_control_service_2_name, ros::Duration(1.0))) return false;
    this->arm_control_client = this->nh.serviceClient<panda_softhand_control::arm_control>(this->arm_control_service_2_name);

    if(!ros::service::waitForService(this->arm_wait_service_2_name, ros::Duration(1.0))) return false;
    this->arm_wait_client = this->nh.serviceClient<panda_softhand_control::arm_wait>(this->arm_wait_service_2_name);

    if(!ros::service::waitForService(this->joint_service_2_name, ros::Duration(1.0))) return false;
    this->joint_client = this->nh.serviceClient<panda_softhand_control::joint_plan>(this->joint_service_2_name);

    if(!ros::service::waitForService(this->pose_service_2_name, ros::Duration(1.0))) return false;
    this->pose_client = this->nh.serviceClient<panda_softhand_control::pose_plan>(this->pose_service_2_name);

    if(!ros::service::waitForService(this->slerp_service_2_name, ros::Duration(1.0))) return false;
    this->slerp_client = this->nh.serviceClient<panda_softhand_control::slerp_plan>(this->slerp_service_2_name);
...
bool PandaSoftHandClient_2::call_hand_plan_service_2(double goal_syn, double goal_duration, trajectory_msgs::JointTrajectory& computed_trajectory){
...
bool PandaSoftHandClient_2::call_hand_control_service_2(trajectory_msgs::JointTrajectory& computed_trajectory){
...
bool PandaSoftHandClient_2::call_hand_wait_service_2(ros::Duration wait_time){
...
bool PandaSoftHandClient_2::call_arm_control_service_2(trajectory_msgs::JointTrajectory& computed_trajectory){
...
bool PandaSoftHandClient_2::call_arm_wait_service_2(ros::Duration wait_time){
...
bool PandaSoftHandClient_2::call_joint_service_2(std::vector<double> joint_goal, std::vector<double> joint_start, trajectory_msgs::JointTrajectory& computed_trajectory){
...
bool PandaSoftHandClient_2::call_pose_service_2(geometry_msgs::Pose goal_pose, geometry_msgs::Pose start_pose, bool is_goal_relative, 
...
bool PandaSoftHandClient_2::call_slerp_service_2(geometry_msgs::Pose goal_pose, geometry_msgs::Pose start_pose, bool is_goal_relative, 
                                            trajectory_msgs::JointTrajectory& computed_trajectory, trajectory_msgs::JointTrajectory past_trajectory){
