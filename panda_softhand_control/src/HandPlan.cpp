...
HandPlan::HandPlan(ros::NodeHandle& nh_, int n_wp_, std::string synergy_joint_name_){
        ...
        this->joints_sub = this->nh.subscribe("/joint_states_desired", 1, &HandPlan::joints_callback, this);
        this->saved_jnt_msg = ros::topic::waitForMessage<sensor_msgs::JointState>("/joint_states_desired", this->nh);
...
void HandPlan::computeTrajectory(double present_syn, double goal_syn, double time){
    ...
    int real_n_wp = std::ceil(std::abs(goal_syn - present_syn) * this->n_wp);
