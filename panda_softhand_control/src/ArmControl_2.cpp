...
ArmControl_2::ArmControl_2(ros::NodeHandle& nh_,
    boost::shared_ptr<actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction>> arm_client_ptr_2){
        
        ROS_INFO("Starting to create ArmControl_2 object");

        // Initializing node handle
        this->nh = nh_;

        // Initializing the arm client
        this->arm_client_ptr2 = arm_client_ptr_2;

        ROS_INFO("Finished creating ArmControl_2 object");
}

ArmControl_2::~ArmControl_2(){
    
    // Nothing to do here yet
}

// This is the callback function of the arm control service
bool ArmControl_2::call_arm_control(panda_softhand_control::arm_control::Request &req, panda_softhand_control::arm_control::Response &res){
...
bool ArmControl_2::sendJointTrajectory(trajectory_msgs::JointTrajectory trajectory){
    
    // Waiting for the arm server to be ready
    if(!this->arm_client_ptr2->waitForServer(ros::Duration(1,0))){
    ...
    ROS_INFO_STREAM("In ArmControl_2::sendJointTrajectory, the traj header stamp is " << trajectory.header.stamp
    ...
    this->arm_client_ptr2->sendGoal(goalmsg);
...
bool ArmControl_2::call_arm_wait(panda_softhand_control::arm_wait::Request &req, panda_softhand_control::arm_wait::Response &res){

     if(!this->arm_client_ptr2->waitForResult(req.wait_duration.data)){
