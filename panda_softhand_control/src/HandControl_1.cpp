...
HandControl_1::HandControl_1(ros::NodeHandle& nh_,
    boost::shared_ptr<actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction>> hand_client_ptr_1){
        
        // Initializing the class node
        this->nh = nh_;

        // Setting the action client
        this->hand_client_ptr1 = hand_client_ptr_1;
}

HandControl_1::~HandControl_1(){
    // Nothing to do here yet
}

// This is the callback function of the hand control service
bool HandControl_1::call_hand_control(panda_softhand_control::hand_control::Request &req, panda_softhand_control::hand_control::Response &res){
    ...
    if(!this->sendHandTrajectory(this->computed_trajectory)){
        ROS_ERROR("Could not send computed trajectory from HandControl_1 object. Returning...");
        res.answer = false;
        return false;
    }
...
bool HandControl_1::sendHandTrajectory(trajectory_msgs::JointTrajectory trajectory){

    // Waiting for the hand server to be ready
    if(!this->hand_client_ptr1->waitForServer(ros::Duration(1,0))){
    ...
    // ROS_INFO_STREAM("In HandControl_1::sendJointTrajectory, the traj header stamp is " << trajectory.header.stamp
    ...
    this->hand_client_ptr1->sendGoal(goalmsg);
...
bool HandControl_1::call_hand_wait(panda_softhand_control::hand_wait::Request &req, panda_softhand_control::hand_wait::Response &res){
    if(!this->hand_client_ptr1->waitForResult(req.wait_duration.data)){
